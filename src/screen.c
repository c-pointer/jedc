...
extern size_t cbrief_flags;						/* ndc: cbrief flags */
extern size_t cbrief_sel_line;					/* ndc: v8+ selection line */
extern size_t cbrief_sel_col;					/* ndc: v8+ selection column */
...
static void display_line (Line *line, int sy, int sx)
{
   ...
   if ((s->hi0 != NULL) && Wants_Attributes)
	...
	/* ndc: column selection -- begin */
 	if ( IS_COLM_SELMODE && s->hi0 != NULL ) {
		int x1 = cbrief_sel_col;
 		int x2 = calculate_column(); // where is the pointer now
		int dx = x2 - x1;
		int y1 = sy; // cbrief_sel_line;
		int y2 = sy;
		int dy = y2 - y1;
 		SLsmg_set_color_in_region(JREGION_COLOR, y1, x1, dy, dx);
		}
/*	 ndc: column selection -- end */
...
//	     if (CBuf->flags & ABBREV_MODE) str = " abbrev";
	     if (CBuf->flags & ABBREV_MODE) str = " abr"; // ndc: change abbrev mark
	     break;
